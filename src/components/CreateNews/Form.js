...
function Form({ dispatch, teacher_add_post }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (title.current.value.length === 0 || title.current.value === null) return;
    if (discription.current.value.length === 0 || discription.current.value === null) return;
    if (link_image.current.value.length === 0 || link_image.current.value === null) return;
  ...
  return (
    <div className="flex container justify-center h-screen">
      <form onSubmit={submit} className="w-full md:w-96 mt-5">
        <div className="mb-3">
